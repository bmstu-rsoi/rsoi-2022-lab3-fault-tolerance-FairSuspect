...
func (rep *CBTicketsRep) GetAll(username string) (objects.TicketArr, error) {
	req, _ := http.NewRequest(http.MethodGet, fmt.Sprintf("%s/api/v1/tickets", rep.endpoint), nil)
	if username != "" {
...
func (rep *CBTicketsRep) Create(flightNumber string, price int, username string) (*objects.TicketCreateResponse, error) {
	...
	req, _ := http.NewRequest(http.MethodPost, fmt.Sprintf("%s/api/v1/tickets", rep.endpoint), bytes.NewBuffer(reqBody))
...
func (rep *CBTicketsRep) Find(ticketUid string) (*objects.Ticket, error) {
	req, _ := http.NewRequest(http.MethodGet, fmt.Sprintf("%s/api/v1/tickets/%s", rep.endpoint, ticketUid), nil)
...
func (rep *CBTicketsRep) Delete(ticketUid string) error {
	req, _ := http.NewRequest(http.MethodDelete, fmt.Sprintf("%s/api/v1/tickets/%s", rep.endpoint, ticketUid), nil)
