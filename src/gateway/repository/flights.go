...
func (rep *CBFlightsRep) GetAll(page int, pageSize int) (*objects.PaginationResponse, error) {
	req, _ := http.NewRequest(http.MethodGet, fmt.Sprintf("%s/api/v1/flights", rep.endpoint), nil)
	q := req.URL.Query()
...
func (rep *CBFlightsRep) Find(flightNumber string) (*objects.FlightResponse, error) {
	req, _ := http.NewRequest(http.MethodGet, fmt.Sprintf("%s/api/v1/flights/%s", rep.endpoint, flightNumber), nil)
